...
# Check if the enterprise-17.0 directory already exists under /opt/odoo/
if ! sudo [ -d "/opt/odoo/enterprise-17.0" ]; then
    # If the directory does not exist, check if enterprise-17.0.zip exists in the same directory as the script
    if [ ! -f "enterprise-17.0.zip" ]; then
        echo -e "${RED}Error: The file 'enterprise-17.0.zip' is missing.${RESET}"
        echo -e "${RED}Please place the 'enterprise-17.0.zip' file in the same directory as this script and try again.${RESET}"
        exit 1
    fi
fi
...
if ! sudo [ -d "/opt/odoo/enterprise-17.0" ]; then
    sudo mv enterprise-17.0.zip /opt/odoo/
    sudo su - odoo << EOF
    unzip /opt/odoo/enterprise-17.0.zip
EOF
else
...
if [ ! -d "design-themes" ]; then
    git clone --depth 1 --branch 17.0 https://github.com/odoo/design-themes.git
else
...
db_password = False
#dbfilter = ^db$
addons_path = /opt/odoo/enterprise-17.0, /opt/odoo/odoo/addons, /opt/odoo/design-themes
proxy_mode = True
http_port = 8069
https_port = 443
